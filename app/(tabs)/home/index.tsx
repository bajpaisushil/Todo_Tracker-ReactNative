...
//     return true; // Return true to indicate that the back press event has been handled
//   };
//   return (
//     <>
//       <View
...
//         }
//         visible={isVisible}
//         onTouchOutside={() => setIsVisible((prev) => !prev)}
//       >
...
const index = () => {
  ...
  const addTodo = async () => {
    try {
      ...
      axios
        .post(`http://192.168.0.106:5000/todos/${userId}`, todoData)
        .then((response) => {
  ...
  const getUserTodos = async () => {
    try {
      const response = await axios.get(
        `http://192.168.0.106:5000/users/${userId}/todos`
      );
  ...
  const markTodoAsCompleted = async (todoId: any) => {
    try {
      ...
      const response = await axios.patch(
        `http://192.168.0.106:5000/todos/${todoId}/complete`
      );
  ...
  return (
    <>
      ...
      <BottomModal
        ...
        onHardwareBackPress={() => {setModalVisible(!isModalVisible); return true;}}
